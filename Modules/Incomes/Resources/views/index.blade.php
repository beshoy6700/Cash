...
@section('title','قائمة تصنيف الايرادات')
@section('content')
    <div class="container">
                ...
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title text-center" id="basic-layout-round-controls">
                            <strong>قائمة تصنيف الايرادات</strong></h4>
                        <div>
                            <br>
                            <a href="{{route('incomes.create')}}" class="btn btn-danger round btn-sm">
                                ...
                                @isset($incomes)
                                    @foreach($incomes as $key => $income)
                                        <tr>
                                            ...
                                            <td>{{$income -> count_sub()}}</td>
                                            <td>
                                                @if($income->getStatus() == 'مفعل')
                                                    <span class="badge bg-success">{{$income->getStatus()}}</span>
                                                @else
                                                    <span class="badge bg-danger">{{$income->getStatus()}}</span>
                                                @endif
                                            </td>
                                            <td>
                                                <div class="btn-group" role="group"
                                                    ...
                                                    <a href="{{route('incomes.status', $income->id)}}"
                                                    ...
                                                    {!! Form::open(['method'=>'DELETE','class'=>'form-check-inline','route'=>['incomes.destroy',$income->id] ]) !!}
